...
public class HibernateDistributionDAO implements DistributionDAO {
	...
	public List<Object[]> loadCompleteDistributionData(List<Integer> taxonIdList) {
		Query query = sessionFactory
				.getCurrentSession()
				.createSQLQuery(
						"SELECT distribution.taxonid, region.iso3166_2, region.region, region.iso3166_1, distributionstatus.occurrencestatus, distributionstatus.establishmentmeans, reference.reference, reference.url, excludedcode.excludedcode"
								...
								+ " AND reference.id = distribution.referenceid" + " AND excludedcode.id = distribution.excludedcodeid")
				.addScalar("taxonid", IntegerType.INSTANCE).addScalar("iso3166_2", StringType.INSTANCE).addScalar("region", StringType.INSTANCE)
				.addScalar("iso3166_1", StringType.INSTANCE).addScalar("occurrencestatus", StringType.INSTANCE)
				.addScalar("establishmentMeans", StringType.INSTANCE).addScalar("reference", StringType.INSTANCE)
				.addScalar("url", StringType.INSTANCE).addScalar("excludedcode", StringType.INSTANCE);
	 ...
	 * @param taxonId
	 * @param computedMap
	 *            map <regionId,distributionstatusid>. The distributionstatusid will be changed if the current on is higher than the one for this
	 *            taxon.
	 * @return
	 */
	// public Map<Integer,Integer> getComputedDistribution(Integer taxonId, Map<Integer,Integer> computedMap){
	// Query query =
	// sessionFactory.getCurrentSession().createSQLQuery("SELECT regionid, distributionstatusid FROM distribution WHERE distribution.taxonid =:id").addScalar("regionid",IntegerType.INSTANCE).addScalar("distributionstatusid",IntegerType.INSTANCE);
