...
public class HibernateTaxonomyDAO implements TaxonomyDAO {
	...
	public List<Integer> getAcceptedChildrenIdList(Integer taxonId) {
		...
		Query query = hibernateSession
				.createSQLQuery("SELECT taxonomy.childid FROM taxonomy,taxon WHERE taxonomy.childid = taxon.id AND taxonomy.parentid = :id AND taxon.statusid = :statusid");
	...
	public List<Integer> getSynonymChildrenIdList(List<Integer> taxonIdList) {
		Session hibernateSession = sessionFactory.getCurrentSession();
		Query query = hibernateSession
				.createSQLQuery("SELECT taxonomy.childid FROM taxonomy,taxon WHERE taxonomy.parentid IN (:taxonid) AND taxon.id = taxonomy.childid AND taxon.statusid = :statusid");
	...
	protected void internalGetChildrenIdSet(Integer taxonId, boolean recursive, LinkedHashSet<Integer> childrenIdSet) {
		Session hibernateSession = sessionFactory.getCurrentSession();
		Query query = hibernateSession
				.createSQLQuery("SELECT taxonomy.childid FROM taxonomy, taxon, rank WHERE taxonomy.childid = taxon.id AND taxonomy.parentid = :id AND taxon.rankid = rank.id ORDER BY rank.sort ASC");
		query.setParameter("id", taxonId);
	...
	private void internalBuildNestedSets(Integer taxonId, AtomicInteger currCounter, Integer parentID, Integer depth) {
		...
		SQLQuery updateCmd = sessionFactory.getCurrentSession().createSQLQuery(
	...
	public List<Integer> getAcceptedChildrenIdListFromNestedSets(Integer taxonId) {
		...
		SQLQuery query = hibernateSession
				.createSQLQuery("SELECT taxonid FROM lookup as child, (SELECT _left,_right FROM lookup where taxonid = :id) as parent "
	...
	public List<TaxonLookupModel> getAcceptedChildrenListFromNestedSets(Integer taxonId, String[] acceptedRanks) {
		...
		Session hibernateSession = sessionFactory.getCurrentSession();
		SQLQuery query = hibernateSession
				.createSQLQuery("SELECT * FROM lookup as child, (SELECT _left,_right FROM lookup where taxonid = :id) as parent "
