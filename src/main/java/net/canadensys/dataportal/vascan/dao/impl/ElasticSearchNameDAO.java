...
public class ElasticSearchNameDAO implements NameDAO {
		...
		SearchRequestBuilder srb = client
				...
				.setQuery(
						QueryBuilders
								...
								.should(QueryBuilders.boolQuery().should(QueryBuilders.matchQuery(TAXON_NAME_NGRAM_FIELD, text))
										.should(QueryBuilders.matchQuery(TAXON_NAME_EPITHET_FIELD, text)))
								.should(QueryBuilders.matchQuery(TAXON_NAME_GENUS_FIRST_LETTER_FIELD, text))).setSize(pageSize)
				.addSort(SortBuilders.scoreSort()).addSort(SortBuilders.fieldSort(TAXON_NAME_FIELD).order(SortOrder.ASC)).setTypes(TAXON_TYPE);
	...
	public List<NameConceptModelIF> searchVernacular(String text) {
		SearchRequestBuilder srb = client
				...
				.setSearchType(SearchType.DFS_QUERY_THEN_FETCH)
				.setQuery(
						QueryBuilders.boolQuery().should(QueryBuilders.matchQuery(VERNACULAR_NAME_FIELD, text))
								.should(QueryBuilders.matchQuery(VERNACULAR_NAME_NGRAM_FIELD, text))
								.should(QueryBuilders.matchQuery(VERNACULAR_NAME_SPLIT_NGRAM_FIELD, text))).setSize(pageSize)
				.addSort(SortBuilders.scoreSort()).addSort(SortBuilders.fieldSort(VERNACULAR_NAME_FIELD).order(SortOrder.ASC))
				...
				.setQuery(
						QueryBuilders
								...
								.should(QueryBuilders.constantScoreQuery(
										QueryBuilders.boolQuery().should(QueryBuilders.matchQuery(TAXON_NAME_NGRAM_FIELD, text))
												.should(QueryBuilders.matchQuery(TAXON_NAME_EPITHET_FIELD, text))).boost(1))
								...
								.should(QueryBuilders.constantScoreQuery(
										QueryBuilders.boolQuery().should(QueryBuilders.matchQuery(VERNACULAR_NAME_NGRAM_FIELD, text))
												.should(QueryBuilders.matchQuery(VERNACULAR_NAME_SPLIT_NGRAM_FIELD, text))).boost(1)))
				.setSize(pageSize).addSort(SortBuilders.scoreSort()).addSort(SortBuilders.fieldSort(SORT_NAME_FIELD).order(SortOrder.ASC));
	}
		...
		return client
				...
				.setQuery(
						QueryBuilders
								.boolQuery()
								.disableCoord(true)
								// ignore coord similarity
								.should(QueryBuilders.constantScoreQuery(QueryBuilders.matchQuery(TAXON_NAME_FIELD, text)).boost(1))
								.should(QueryBuilders.constantScoreQuery(
										QueryBuilders.boolQuery().should(QueryBuilders.matchQuery(TAXON_NAME_EPITHET_FIELD, text))
												.should(QueryBuilders.matchQuery(TAXON_NAME_GENUS_FIRST_LETTER_FIELD, text))).boost(0.5f))
								.should(QueryBuilders.constantScoreQuery(QueryBuilders.matchQuery(VERNACULAR_NAME_FIELD, text)).boost(1))
								// constant query with 0.5 boost only if we do not have a complete match so scores will be comparable with taxon
								.should(QueryBuilders.constantScoreQuery(
										QueryBuilders.boolQuery().mustNot(QueryBuilders.matchQuery(VERNACULAR_NAME_FIELD, text))
												.must(QueryBuilders.matchQuery(VERNACULAR_NAME_SPLIT_NAME_FIELD, text))).boost(0.5f)))
				.setSize(pageSize).addSort(SortBuilders.scoreSort()).addSort(SortBuilders.fieldSort(SORT_NAME_FIELD).order(SortOrder.ASC));
	}
	...
	public boolean indexTaxon(TaxonLookupModel tlm) {
		...
		IndexResponse response = client.prepareIndex(INDEX_NAME, TAXON_TYPE).setSource(sourceData).setId(Integer.toString(tlm.getTaxonId()))
				.execute().actionGet();
