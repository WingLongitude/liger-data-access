...
public class HibernateTaxonDAO implements TaxonDAO {
	...
	private enum RegionCriterionOperatorEnum {
		OR, AND
	};
	...
	public boolean deleteTaxon(Integer taxonId) {
		...
		if (taxonModelToDelete != null) {
			...
			Query query = hibernateSession.createSQLQuery("SELECT count(childid)noc FROM taxonhybridparent WHERE parentid = :taxonid")
					.addScalar("noc", BigIntegerType.INSTANCE).setParameter("taxonid", taxonId);
	...
	public List<TaxonModel> loadTaxonByName(String taxonCalculatedName) {
		Criteria searchCriteria = sessionFactory.getCurrentSession().createCriteria(TaxonModel.class).createCriteria("lookup")
				.add(Restrictions.like("calname", taxonCalculatedName));
		return (List<TaxonModel>) searchCriteria.list();
	}
	...
	public Iterator<TaxonLookupModel> loadTaxonLookup(int limitResultsTo, String habitus, int taxonid, RegionQueryPart rqp, String[] status,
		...
		return new ScrollableResultsIteratorWrapper<TaxonLookupModel>(searchCriteria.scroll(ScrollMode.FORWARD_ONLY),
	...
	public List<Object[]> getAcceptedTaxon(int maximumRank) {
		...
		Query query = hibernateSession
				.createSQLQuery(
						"SELECT taxon.id, lookup.calname, lookup.rank FROM taxon,lookup WHERE taxon.id = lookup.taxonid AND taxon.rankid <= :rankid AND taxon.statusid = :statusid  ORDER BY lookup.calname ASC")
				.addScalar("id", IntegerType.INSTANCE).addScalar("calname", StringType.INSTANCE).addScalar("rank", StringType.INSTANCE)
				.setParameter("rankid", maximumRank).setParameter("statusid", STATUS_ACCEPTED);
	...
	public List<Object[]> loadCompleteTaxonData(List<Integer> taxonIdList) {
		Query query = sessionFactory
				.getCurrentSession()
				.createSQLQuery(
						"SELECT taxon.id, taxon.mdate, lookup.status, taxonomy.parentid, reference.url, reference.reference, lookup.calnameauthor,taxon.author,lookup.rank, N.calnameauthor AS parentFSN, lookup.higherclassification, "
								...
								+ " WHERE taxon.id IN (:id)").addScalar("id", IntegerType.INSTANCE).addScalar("mdate", CalendarType.INSTANCE)
				.addScalar("status", StringType.INSTANCE).addScalar("parentid", IntegerType.INSTANCE).addScalar("url", StringType.INSTANCE)
				.addScalar("reference", StringType.INSTANCE).addScalar("calnameauthor", StringType.INSTANCE).addScalar("author", StringType.INSTANCE)
				.addScalar("rank", StringType.INSTANCE).addScalar("parentfsn", StringType.INSTANCE)
				.addScalar("higherclassification", StringType.INSTANCE).addScalar("class", StringType.INSTANCE)
				.addScalar("_order", StringType.INSTANCE).addScalar("family", StringType.INSTANCE).addScalar("genus", StringType.INSTANCE)
				.addScalar("subgenus", StringType.INSTANCE).addScalar("specificepithet", StringType.INSTANCE)
	...
	private void getTaxonIdTree(int taxonid, List<Integer> idList) {
		...
		try {
			query = hibernateSession
					.createSQLQuery("SELECT childid FROM taxonomy,taxon WHERE parentid = :taxonid AND taxonomy.childid = taxon.id AND taxon.statusid = :statusid");
	 ...
	 * AND (p_ON = 'native' OR p_ON = 'ephemere') AND (p_QC = 'native' OR p_QC = 'ephemere')
	 * AND p_BC <> 'native' AND p_BC <> 'ephemere' AND p_PM <> 'native' AND p_MB <> 'ephemere' [and so on for all provinces & territories, except p_PM
	 * & p_GL] ...
	 * 
	 * @param rqp
	 * @param status
