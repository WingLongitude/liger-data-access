...
public class HibernateOccurrenceAutoCompleteDAO implements OccurrenceAutoCompleteDAO {
	...
	public String getSuggestionsFor(String field, String currValue, boolean useSanitizedValue) {
		Criteria suggestionCriteria = sessionFactory.getCurrentSession().createCriteria(UniqueValuesModel.class).add(Restrictions.eq("key", field))
				.addOrder(Order.desc("occurrence_count")).setMaxResults(10);
