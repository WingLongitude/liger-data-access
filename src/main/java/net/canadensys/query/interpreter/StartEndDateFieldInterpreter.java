...
public class StartEndDateFieldInterpreter implements QueryPartInterpreter {
	...
	public boolean canHandleSearchableField(SearchableField searchableField) {
		return (searchableField.getRelatedFields() != null && searchableField.getRelatedFields().size() >= 3 && searchableField.getType() == null);
	}
			...
			case BETWEEN:
				...
				for (String currRelatedField : relatedFieldList) {
					...
					if (startValueParsed != null && endValueParsed != null) {
						Criterion innerCriterion = Restrictions.and(Restrictions.ge(currRelatedField, startValueParsed),
		...
		switch (searchQueryPart.getOp()) {
			...
			case BETWEEN:
				...
				for (String currRelatedField : relatedFieldList) {
					...
					// could be optional
					if (startValueParsed != null && endValueParsed != null) {
						String innerSql = SQLHelper.and(SQLHelper.ge(currRelatedField, startValueParsed),
