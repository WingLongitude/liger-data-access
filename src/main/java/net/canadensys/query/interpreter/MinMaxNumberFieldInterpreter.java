...
public class MinMaxNumberFieldInterpreter implements QueryPartInterpreter {
	...
	public boolean canHandleSearchableField(SearchableField searchableField) {
		return (searchableField.getRelatedFields() != null && searchableField.getRelatedFields().size() == 2 && searchableField.getType() != null && Number.class
				.isAssignableFrom(searchableField.getType()));
	}
	...
	public Criterion toCriterion(SearchQueryPart searchQueryPart) {
		...
		switch (searchQueryPart.getOp()) {
			case EQ:
				return Restrictions.and(Restrictions.le(searchableField.getRelatedFields().get(0), parsedValueMin),
			...
			case BETWEEN:
				...
				return Restrictions.or(
						Restrictions.and(Restrictions.ge(searchableField.getRelatedFields().get(0), parsedValueMin),
								Restrictions.le(searchableField.getRelatedFields().get(0), parsedValueMax)),
						Restrictions.and(Restrictions.ge(searchableField.getRelatedFields().get(1), parsedValueMin),
