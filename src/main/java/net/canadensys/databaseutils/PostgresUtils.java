...
public class PostgresUtils {
	 ...
	 * 
	 * @param column
	 *            the column to count the number of unique value
	 * @param whereClause
	 *            optional formatted where clause
	 * @param table
	 * @return
	 */
	public static String getOptimizedCountDistinctQuery(String column, String whereClause, String table, String alias) {
		if (StringUtils.isBlank(whereClause)) {
			...
			return "SELECT COUNT(*)" + alias + " FROM(SELECT 1 FROM " + table + " WHERE " + whereClause + " AND " + column + " IS NOT NULL GROUP BY "
					+ column + ")cc";
		}
	}
}
