...
public class NameDAOTest {
	...
	public void setupES() throws Exception {
		...
		catch (IndexMissingException imEx) {
		}// ignore
		URL indexCreationScriptURL = NameDAOTest.class.getResource("/script/vascan/vascan_index_creation.txt");
		client.admin().indices().prepareCreate("vascan").setSource(FileUtils.readFileToString(new File(indexCreationScriptURL.toURI()))).execute()
				.actionGet();

		// TODO : move data to external file
		client.prepareIndex("vascan", "taxon", "951")
				.setSource(
						jsonBuilder().startObject().field("taxonname", "Carex").field("sortname", "Carex").field("status", "accepted")
								.field("namehtml", "<em>Carex</em>").field("namehtmlauthor", "<em>Carex</em> Linnaeus").field("rankname", "genus")
								.endObject()).execute().actionGet();

		client.prepareIndex("vascan", "taxon", "4864")
				.setSource(
						jsonBuilder().startObject().field("taxonname", "Carex feta").field("sortname", "Carex feta").field("status", "accepted")
								.field("namehtml", "<em>Carex feta</em>").field("namehtmlauthor", "<em>Carex feta</em> L.H. Bailey")
								.field("rankname", "species").endObject()).execute().actionGet();

		// add taxon with genus = epithet
		client.prepareIndex("vascan", "taxon", "26305")
				.setSource(
						jsonBuilder().startObject().field("taxonname", "Carex Carex bigelowii × Carex saxatilis var. rhomalea")
								.field("sortname", "Carex Carex bigelowii × Carex saxatilis var. rhomalea").field("status", "synonym")
								...
								.field("rankname", "variety").endObject()).execute().actionGet();

		// Add a hybrid
		client.prepareIndex("vascan", "taxon", "23238")
				.setSource(
						jsonBuilder().startObject().field("taxonname", "×Achnella").field("sortname", "×Achnella").field("status", "accepted")
								.field("namehtml", "<em>×Achnella</em>").field("namehtmlauthor", "<em>×Achnella</em> Barkworth")
								.field("parentid", 746).field("parentnamehtml", "<em>Stipinae</em>").endObject()).execute().actionGet();

		client.prepareIndex("vascan", "taxon", "1941")
				.setSource(
						jsonBuilder().startObject().field("taxonname", "Carex straminea var. mixta").field("sortname", "Carex straminea var. mixta")
								.field("status", "synonym").field("namehtml", "<em>Carex straminea</em> var. <em>mixta</em>")
								.field("namehtmlauthor", "<em>Carex straminea</em> var. <em>mixta</em> L.H. Bailey").field("parentid", 864)
								.field("parentnamehtml", "<em>Carex feta</em>").endObject()).execute().actionGet();

		client.prepareIndex("vascan", "taxon", "5064")
				.setSource(
						jsonBuilder().startObject().field("taxonname", "Carex sabulosa").field("sortname", "Carex sabulosa")
								.field("status", "accepted").field("namehtml", "<em>Carex sabulosa</em>")
								.field("namehtmlauthor", "<em>Carex sabulosa</em> Turczaninow ex Kunth").field("parentid", 2096)
								.field("parentnamehtml", "<em>Carex</em> sect. <em>Racemosae</em>").endObject()).execute().actionGet();

		// add a synonym with 2 parents, not common but legit
		client.prepareIndex("vascan", "taxon", "101010")
				.setSource(
						jsonBuilder().startObject().field("taxonname", "SynonymWithTwoParents").field("sortname", "SynonymWithTwoParents")
								.field("status", "synonym").field("namehtml", "<em>SynonymWithTwoParents</em>")
								.field("namehtmlauthor", "<em>SynonymWithTwoParents</em> Turczaninow ex Kunth").array("parentid", 2096, 2097)
		...
		client.prepareIndex("vascan", "vernacular", "3")
				.setSource(
						jsonBuilder().startObject().field("taxonid", 7174).field("taxonnamehtml", "<em>Picea mariana</em>")
								.field("vernacularname", "épinette").field("sortname", "épinette").endObject()).execute().actionGet();

		client.prepareIndex("vascan", "vernacular", "9583")
				.setSource(
						jsonBuilder().startObject().field("taxonid", 4912).field("taxonnamehtml", "<em>Carex illota</em>")
								.field("vernacularname", "carex sali").field("sortname", "carex sali").endObject()).execute().actionGet();

		client.prepareIndex("vascan", "vernacular", "25445")
				.setSource(
						jsonBuilder().startObject().field("taxonid", 9208).field("taxonnamehtml", "<em>Acer palmatum</em>")
								.field("vernacularname", "Japanese maple").field("sortname", "Japanese maple").endObject()).execute().actionGet();
		// refresh the index
