...
public class OccurrenceAutoCompleteDAOTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void setup() {
		...
		jdbcTemplate
				.update("INSERT INTO unique_values (key,occurrence_count,value,unaccented_value) VALUES ('country',8,'Côte d''Ivoire','cote d''ivoire')");
	...
	public void testUnaccentedSearch() {
		...
		try {
			...
			for (int i = 0; i < jsonRows.length(); i++) {
				assertTrue(expectedCountryList.contains(((JSONObject) jsonRows.get(i)).get("value")));
			}
		}
		catch (Exception e) {
			fail();
