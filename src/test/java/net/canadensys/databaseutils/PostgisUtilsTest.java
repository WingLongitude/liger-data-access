...
public class PostgisUtilsTest {
	...
	public void testGetCentroidSQLQuery() {
		...
		assertEquals("SELECT ST_AsText(st_centroid(st_collect(the_geom))) point FROM table WHERE a = '3'", sql);
	}
	...
	public void testGetCentroidSQLQueryWithoutWhere() {
		...
		assertEquals("SELECT ST_AsText(st_centroid(st_collect(the_geom))) point FROM table", sql);
	}
	...
	public void testGetInsideEnvelopeSQLClause() {
		...
		assertEquals("the_geom && ST_MakeEnvelope(29.53,75.15,29,77,4326)", PostgisUtils.getInsideEnvelopeSQLClause("the_geom", envelope, false));
